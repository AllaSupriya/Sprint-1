...
import com.ec.onlineplantnursery.customer.service.ICustomerService;
...
Logger log = org.slf4j.LoggerFactory.getLogger(NurseryRestController.class);
	...
	public Order newOrder(@RequestBody @Valid Order o) throws ResourceNotFoundException {
		...
		return oservice.addOrder(o).get();
	}
	
	@ApiOperation(value = "Planter post mapping" , response = Planter.class)
	@PostMapping("/insert/planter/{pid}/{sid}")
	public Planter insertPlanter(@RequestBody @Valid Planter p,@PathVariable int pid,@PathVariable int sid) throws ResourceNotFoundException {
		log.info("inside insert planter");
		return pservice.addPlanter(p,pid,sid);
	}
	...
	public List<Planter> viewByIdPlanter(@PathVariable String plshape) throws ResourceNotFoundException{
		log.info("Get Planter information by id");
	...
	public List<Planter> viewPlantersListByOrderId(@PathVariable int id) throws ResourceNotFoundException {
		...
		return oservice.viewPlanterByOrderId(id);
	}
